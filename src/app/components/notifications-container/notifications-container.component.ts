...
export class NotificationsContainerComponent implements OnInit {
  ...
  setSub() {
    this.scrollService.getObservable().subscribe(status => {
      if (status && this.isBottomOfScreen) { // && this.isScrollingBottom
  ...
  // scrolling event listener, checks if the target element is in view or if the bottom of the screen has been reached
  scrollEventCheck() {
  ...
  deleteHandler(id: string) {
    const index = this.notifications.findIndex((booking) => booking.id === id)
